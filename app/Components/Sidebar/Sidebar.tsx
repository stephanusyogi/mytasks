...
import { bars, logout,arrowLeft } from '@/app/utils/Icons';
...
function Sidebar() {
  ...
  return (
    <SidebarStyled theme={theme} collapsed={collapsed ? 1 : 0}>
      ...
      <div className="profile">
        ...
        <div className="image">
          {
            imageUrl && (
              <Image width="50" height="50" src={imageUrl} alt='avatar' style={{ width:'auto' }}/>
            )
          }
        </div>
      ...
      <ul className="nav-items">
        {menu.map((item)=>{
          return (
            <li key={item.id} onClick={()=>handleClick(item.link)} className={`nav-item ${pathname === item.link ? 'active' : ''}`}>
              {item.icon}
